...
int main()
{
    ...
    while (1) {
        ...
        if (0 == state.EX.nop)
        {
            ...
            if (state.EX.INS == "beq")	//Branch On Equal if(R[rs]==R[rt]) PC=PC+4+BrachAddr
            {
                ...
                if (myRF.readRF(state.EX.Rs) != myRF.readRF(state.EX.Rt))
                {
                    ...
                    newState.IF.PC = state.IF.PC.to_ulong() -4 + bitset<30>(sign_extend(Imm).to_string().substr(2,30)).to_ulong()*4;
                    newState.IF.nop = 0;
