        ...
        if (0 == state.EX.nop) {
            ...
            if ((0 == state.MEM.nop) && (0 == state.MEM.rd_mem) && (0 == state.MEM.wrt_mem) &&
                ...
                if ((0 == state.EX.is_I_type) && (1 ==
                                                  state.EX.wrt_enable))   // || (1 == state.EX.wrt_mem))   //addu, subu, for sw, we choose MEM-MEM but EX-EX
                {
            ...
            if (state.EX.INS == "beq") {
                bitset<32> a = myRF.readRF(state.EX.Rs);
                bitset<32> b = myRF.readRF(state.EX.Rt);
...
//                }
                if (a != b) {
                    ...
                    newState.IF.PC = bitset<32>(state.IF.PC.to_ulong() - 4 +
                                                bitset<30>(sign_extend(state.EX.Imm).to_string().substr(2,
                                                                                                        30)).to_ulong() *
                                                4);
                    newState.IF.nop = 0;
...
//            if (opcode == "000100")    //Branch On Equal if(R[rs]==R[rt]) PC=PC+4+BrachAddr
//            {
//                newState.MEM.INS = "beq";
//
